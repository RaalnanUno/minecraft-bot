...
// Bravo help
// Bravo recipe stick
// Bravo recipe diamond axe
// Bravo craft stick
// Bravo craft crafting table
// Bravo craft wooden axe
// Bravo craft diamond axe
// Bravo first tool
...
const state = {
  mode: 'idle', // idle | moving | mining | collecting | fighting | chatting | crafting | auto_follow_bravo | auto_engage | auto_mine | rally
...
function setMode(mode) {
  state.mode = mode
  console.log(`[Bravo] Mode set to: ${mode}`)
}
...
function markActive(reason = 'activity') {
  ...
  console.log(`[Bravo] Active: ${reason}`)
}
...
function hardStopMotion() {
  try {
    ...
    console.log('[Bravo] Could not clear goal:', err.message)
  }

  try {
    bot.pathfinder.stop()
  } catch (err) {
    console.log('[Bravo] Could not stop pathfinder:', err.message)
  }

  try {
    bot.clearControlStates()
  } catch (err) {
    console.log('[Bravo] Could not clear control states:', err.message)
  }
}
...
function setLeaderName(playerName) {
  ...
  console.log(`[Bravo] Leader set to: ${playerName}`)
}
...
function loadCraftingData() {
  try {
    ...
    console.log(
      `[Bravo] Loaded crafting recipes: ${Object.keys(state.craftingData.items).length} item(s)`
    )
  } catch (err) {
    console.error('[Bravo] Failed to load craftingRecipes.json:', err.message)
    state.craftingData = {
      items: {},
      aliases: {},
      groups: {}
...
async function craftRecipeByName(recipeName, options = {}) {
  ...
  try {
    ...
    console.log('[Bravo] Could not equip crafted item:', err.message)
  }
...
function getInventorySummary() {
  ...
  if (!items || items.length === 0) {
    return 'Bravo inventory is empty.'
  }
  ...
  if (items.length > 8) {
    return `Bravo has: ${summary}, and more.`
  }

  return `Bravo has: ${summary}.`
}
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are Bravo, an aggressive Minecraft combat escort for Bravo the miner. Keep replies short, direct, and professional.'
        },
        {
          role: 'user',
...
async function moveNearPlayer(playerName, options = {}) {
  ...
  if (!player || !player.entity) {
    if (announce) {
      bot.chat(`Bravo can't find ${playerName}.`)
    }
    return false
  }
  ...
  if (announce) {
    bot.chat(`Bravo moving to ${playerName}.`)
  }
...
function showHelp() {
  bot.chat(
    'Bravo commands: help, materials, nearby, follow me, come here, rally, mine stone, collect items, fight, first tool, inventory, recipe <item>, craft <item>'
  )
}

function getThreatNearBravo() {
  ...
  const hostiles = Object.values(bot.entities)
    .filter(entity => {
      ...
      return bravo.position.distanceTo(entity.position) <= PROTECT_RADIUS
    })
    .sort((a, b) => bravo.position.distanceTo(a.position) - bravo.position.distanceTo(b.position))
...
async function runAutonomyTick() {
  ...
  try {
    ...
    const threat = getThreatNearBravo()
    if (threat) {
    ...
    if (bravo) {
      const distToBravo = bot.entity.position.distanceTo(bravo.position)
      if (distToBravo > FOLLOW_DISTANCE + 1.5) {
        const taskToken = nextTaskToken()
        clearCancelRequest()
        setMode('auto_follow_bravo')
        markActive('auto_follow_bravo_started')
    ...
    console.error('[Bravo] Autonomy tick error:', err)
    setMode('idle')
    clearCancelRequest()
  } finally {
...
async function handleCommand(username, prompt) {
  ...
  if (normalized === 'follow me') {
    ...
    bot.chat(`Bravo will stay with you, ${username}.`)
    return
  }
  ...
  if (normalized === 'rally') {
    ...
    await runCommandTask('rally', 'rally_command', async ({ shouldCancel }) => {
      try {
        ...
        bot.chat('Bravo rally complete. Holding position.')
      } catch (err) {
        console.error('[Bravo] Rally error:', err)
        bot.chat('Bravo had trouble rallying.')
      }
    })
    return
  }
  ...
  if (normalized === 'come here') {
    await runCommandTask('moving', 'come_here_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Bravo] Movement error:', err)
        bot.chat('Bravo had trouble reaching you.')
      }
    })
    return
  }
  ...
  if (normalized === 'fight' || normalized === 'attack mob' || normalized === 'attack mobs') {
    await runCommandTask('fighting', 'fight_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Bravo] Fight error:', err)
        bot.chat('Bravo had trouble fighting the hostile mob.')
      }
    })
    return
  }
  ...
  if (normalized === 'collect items' || normalized === 'pick up items') {
    await runCommandTask('collecting', 'collect_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Bravo] Collect items error:', err)
        bot.chat('Bravo had trouble collecting items.')
      }
    })
    return
  }
  ...
  if (
    ...
    await runCommandTask('crafting', 'bootstrap_first_tool_command', async ({ shouldCancel }) => {
      try {
        ...
        console.log('[Bravo] bootstrapFirstTool result:', result)

        if (!result.success && !result.canceled) {
          bot.chat(`Bravo could not finish first tool. Reason: ${result.reason}.`)
        }
      } catch (err) {
        console.error('[Bravo] bootstrapFirstTool error:', err)
        bot.chat(`Bravo had trouble making first tool: ${err.message}`)
      }
    })
    return
  }
  ...
  if (normalized === 'mine stone' || normalized === 'mine rock') {
    await runCommandTask('mining', 'mine_stone_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Bravo] Mine stone error:', err)
        bot.chat('Bravo had trouble mining stone.')
      }
    })
    return
  }
  ...
  if (normalized.startsWith('craft ')) {
    ...
    if (!requestedItem) {
      bot.chat('Say something like: Bravo craft wooden axe')
      return
    }
    ...
    await runCommandTask('crafting', 'craft_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Bravo] Craft command error:', err)
        bot.chat(`Bravo had trouble crafting ${requestedItem}: ${err.message}`)
      }
    })
    return
  }
  ...
  await runCommandTask('chatting', 'chat_command', async ({ shouldCancel }) => {
    try {
      ...
      console.error('[Bravo] Ollama error:', err)
      bot.chat('Bravo could not reach Ollama.')
    }
  })
}
...
bot.on('spawn', () => {
  ...
  bot.chat('Bravo online. Combat, mining, and recipe crafting mode active.')

  markActive('spawn')

  setInterval(() => {
    runAutonomyTick().catch(err => console.error('[Bravo] Autonomy loop failure:', err))
  }, AUTO_TICK_MS)
})

bot.on('chat', async (username, message) => {
  console.log(`[Bravo] ${username}: ${message}`)

  if (username === bot.username) return

  const lower = message.toLowerCase()

  if (!lower.startsWith('bravo ')) return

  const prompt = message.slice(6).trim()

  if (!prompt) {
    bot.chat('Say something like: Bravo help')
    return
  }
...
bot.on('path_update', results => {
  console.log(`[Bravo] Path update: ${results.status}`)
})

bot.on('goal_reached', () => {
  console.log('[Bravo] Goal reached.')
...
bot.on('error', err => console.log('[Bravo] Bot error:', err))
bot.on('end', () => console.log('[Bravo] Bot disconnected'))
bot.on('kicked', reason => console.log('[Bravo] Bot kicked:', reason))
```
