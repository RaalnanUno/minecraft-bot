...
I would like to rewrite the bots to use shared functions where possible, so we don't have so much repeating.
Also, there should be 2 bots.
...
// Alpha help
// Alpha recipe stick
// Alpha recipe diamond axe
// Alpha craft stick
// Alpha craft crafting table
// Alpha craft wooden axe
// Alpha craft diamond axe
// Alpha first tool
...
function loadCraftingData() {
  try {
    ...
    console.log(
      `[Alpha] Loaded crafting recipes: ${Object.keys(state.craftingData.items).length} item(s)`
    )
  } catch (err) {
    console.error('[Alpha] Failed to load craftingRecipes.json:', err.message)
    state.craftingData = {
      items: {},
      aliases: {},
      groups: {}
...
async function craftRecipeByName(recipeName, options = {}) {
  ...
  try {
    ...
    console.log('[Alpha] Could not equip crafted item:', err.message)
  }
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are Alpha, an aggressive Minecraft combat escort for Alpha the miner. Keep replies short, direct, and professional.'
        },
        {
          role: 'user',
...
function showHelp() {
  bot.chat(
    'Alpha commands: help, materials, nearby, follow me, come here, rally, mine stone, collect items, fight, first tool, inventory, recipe <item>, craft <item>'
  )
}
...
async function handleCommand(username, prompt) {
  ...
  if (
    ...
    await runCommandTask('crafting', 'bootstrap_first_tool_command', async ({ shouldCancel }) => {
      try {
        ...
        console.log('[Alpha] bootstrapFirstTool result:', result)

        if (!result.success && !result.canceled) {
          bot.chat(`Alpha could not finish first tool. Reason: ${result.reason}.`)
        }
      } catch (err) {
        console.error('[Alpha] bootstrapFirstTool error:', err)
        bot.chat(`Alpha had trouble making first tool: ${err.message}`)
      }
    })
    return
  }
  ...
  if (normalized.startsWith('craft ')) {
    ...
    if (!requestedItem) {
      bot.chat('Say something like: Alpha craft wooden axe')
      return
    }
    ...
    await runCommandTask('crafting', 'craft_command', async ({ shouldCancel }) => {
      try {
        ...
        console.error('[Alpha] Craft command error:', err)
        bot.chat(`Alpha had trouble crafting ${requestedItem}: ${err.message}`)
      }
    })
    return
  }
...
bot.on('spawn', () => {
  ...
  bot.chat('Alpha online. Combat, mining, and recipe crafting mode active.')
